...
type CreateInvoiceApiResponse = Partial<CreateInvoiceResponse> & {
  error?: string;
};
...
export default function CreatePage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = (await res.json()) as CreateInvoiceApiResponse;
      ...
      if (!data.slug) {
        setError("Invoice was created, but the response did not include a link.");
        return;
      }

      window.location.href = `/i/${data.slug}`;
    } catch (err) {
      console.error(err);
      ...
      <div className="w-full max-w-xl space-y-6">
        <header className="space-y-2">
          ...
          <p className="text-sm text-gray-600">
            Create a shareable on-chain Bitcoin invoice with BitGill.
          </p>
        </header>
